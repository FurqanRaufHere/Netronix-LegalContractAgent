...
def call_groq_chat(
    ...
    for attempt in range(1, max_retries + 2):  # +1 for first attempt
        try:
            ...
            last_exc = e
            logger.warning("Network error on GROQ attempt %d/%d: %s", attempt, max_retries + 1, str(e))
        except ValueError as e:
            # This includes JSON parse errors and no content
            last_exc = e
            logger.warning("GROQ returned invalid content on attempt %d: %s", attempt, str(e))
            # If it's a content-parsing error, no point retrying repeatedly; break early
            break
